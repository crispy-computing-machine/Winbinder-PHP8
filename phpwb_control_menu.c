	...
	if (Z_TYPE_P(zarray) == IS_ARRAY)
	{
		...
		for (i = 0; i < nelem; i++)
		{
			...
			case IS_ARRAY: // A menu item is an array inside an array
				...
				pitem[i]->pszCaption = Utf82WideChar(TEXT(pitem[i]->pszCaption), 0);
				pitem[i]->pszHint = Utf82WideChar(TEXT(pitem[i]->pszHint), 0);
				pitem[i]->pszImage = Utf82WideChar(TEXT(pitem[i]->pszImage), 0);
