	...
	if (Z_TYPE_P(zarray) == IS_ARRAY)
	{
		...
		for (i = 0; i < nelem; i++)
		{
			...
			case IS_ARRAY: // Toolbar button
				...
				pitem[i]->pszCaption = Utf82WideChar((const char *)pitem[i]->pszCaption, Z_STRLEN_P((const char *)pitem[i]->pszCaption));
				pitem[i]->pszHint = Utf82WideChar((const char *)pitem[i]->pszHint, Z_STRLEN_P((const char *)pitem[i]->pszCaption)); 
