...
ZEND_FUNCTION(wb_create_treeview_item)
{
	...
	TCHAR *wcs = 0;
	LONG_PTR ret;
	zend_long where_len ,img1_len, img2_len, insertiontype_len;
	...
	ZEND_PARSE_PARAMETERS_START(2, 7)
		...
		Z_PARAM_LONG_OR_NULL(where, where_len)
		Z_PARAM_LONG_OR_NULL(img1, img1_len)
		Z_PARAM_LONG_OR_NULL(img2, img2_len)
		Z_PARAM_LONG_OR_NULL(insertiontype, insertiontype_len)
	ZEND_PARSE_PARAMETERS_END();
