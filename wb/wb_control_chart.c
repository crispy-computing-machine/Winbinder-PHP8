	...
	case WM_PAINT:
		if (pData)
		{
			...
			if (pData->bShowAxis)
			{
				...
				_stprintf(szAxis, TEXT("%.2f"), pData->dAxisMax);
				TextOut(hdc, 4, rcPlot.top - 6, szAxis, (int)_tcslen(szAxis));
				_stprintf(szAxis, TEXT("%.2f"), pData->dAxisMin);
			...
			if (pData->ppszLabels && pData->nLabelCount > 0)
			{
				for (i = 0; i < pData->nLabelCount && i < pData->nPointCount; i++)
				{
					...
					TextOut(hdc, x - (sz.cx / 2), rcPlot.bottom + 4, pData->ppszLabels[i], (int)_tcslen(pData->ppszLabels[i]));
				}
			}
			for (i = 0; i < pData->nSeriesCount; i++)
			{
